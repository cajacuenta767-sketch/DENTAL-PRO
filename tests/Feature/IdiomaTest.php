...
class IdiomaTest extends CasoClinico
{
    ...
    {
        $this->from('/olvide-password')
            ->post('/olvide-password', ['email' => 'nadie@ejemplo.test']);

        $mensaje = session('errors')->getBag('default')->first('email');

        $this->assertStringNotContainsString('passwords.', $mensaje);
        $this->assertSame('No encontramos ninguna cuenta con ese correo electrónico.', $mensaje);
    }
