...
<div class="row g-3">
    <div class="col-lg-7">
        <div class="row row-cards mb-3">
            ...
            <div class="col-sm-4">
                <x-kpi titulo="Efectivo del día" :valor="number_format($resumen['efectivo'], 2).' '.$ajustes->divisa" icono="ti ti-cash" color="azure" />
            </div>
            <div class="col-sm-4">
                <x-kpi titulo="Recibos vigentes" :valor="$resumen['recibos']" icono="ti ti-receipt" color="indigo"
                       :pie="$resumen['anulados'] ? $resumen['anulados'].' anulado(s)' : null" />
            </div>
        </div>
        ...
        <div class="card mb-3">
            ...
            <div class="table-responsive">
                <table class="table table-vcenter card-table">
                    ...
                    <tfoot>
                        <tr><th>Total</th><th class="text-end">{{ number_format($resumen['total'], 2) }} {{ $ajustes->divisa }}</th></tr>
                    </tfoot>
                </table>
            </div>
        </div>
    ...
    <div class="col-lg-5">
        <div class="card">
            <div class="card-header"><h3 class="card-title"><i class="ti ti-cash-register me-2"></i>Cerrar caja</h3></div>
            <form method="POST" action="{{ route('admin.caja.cerrar') }}">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <div class="text-secondary small text-uppercase">Efectivo esperado</div>
                        <div class="arqueo-total" id="arqueo-esperado" data-efectivo="{{ $resumen['efectivo'] }}">
                            {{ number_format($fondoSugerido + $resumen['efectivo'], 2) }} {{ $ajustes->divisa }}
                        </div>
                        <div class="text-secondary small">Fondo inicial + {{ number_format($resumen['efectivo'], 2) }} de efectivo cobrado.</div>
                    </div>

                    <x-campo nombre="efectivo_contado" etiqueta="Efectivo contado" requerido ayuda="Lo que hay físicamente en la caja.">
                ...
                <div class="card-footer">
                    <button class="btn btn-primary w-100" {{ $cierreExistente ? 'disabled' : '' }}
                            onclick="return confirm('¿Cerrar la caja del {{ $fecha->format('d/m/Y') }}? No podrás volver a cerrarla.')">
                        <i class="ti ti-lock me-1"></i>Cerrar caja del {{ $fecha->format('d/m/Y') }}
                    </button>
                </div>
            </form>
        </div>
